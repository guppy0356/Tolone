...
- A query gated by `enabled` reports `isPending` even while disabled — it has no data and none was requested — so there `isPending` means "not asked yet *or* loading". Use `isLoading` (`isPending && isFetching`) when the flag drives a Skeleton
